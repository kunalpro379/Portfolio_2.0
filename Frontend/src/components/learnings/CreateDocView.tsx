...
interface CreateDocViewProps {
  password: string;
}

export function CreateDocView({ password }: CreateDocViewProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch(`${config.apiUrl}/documentation/create`, {
        method: "POST",
      ...
      if (!res.ok) throw new Error(data.message || "Failed to create documentation");
      queryClient.invalidateQueries({ queryKey: ["docs"] });
      navigate({ to: `/learnings/docs/${data.doc._id}` });
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to create documentation");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F0]">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <button
          onClick={() => navigate({ to: "/learnings", search: { tab: "docs" } })}
        ...
        <h1 className="text-2xl font-bold text-black mb-6">Create Documentation</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={() => navigate({ to: "/learnings", search: { tab: "docs" } })} className="flex-1 border-2 border-black/25 py-3 text-sm font-semibold uppercase tracking-wide hover:bg-black/5">
              Cancel
            </button>
