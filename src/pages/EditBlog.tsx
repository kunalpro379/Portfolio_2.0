...
const blogEndpoints = {
  byId: (id: string) => `${API_BASE_URL}${API_ENDPOINTS.blogs}/${id}`,
  mdContent: (id: string) => `${API_BASE_URL}${API_ENDPOINTS.blogs}/${id}/md-content`,
  ...
  assetByIndex: (id: string, index: number) => `${API_BASE_URL}${API_ENDPOINTS.blogs}/${id}/assets/${index}`,
  assetName: (id: string, index: number) => `${API_BASE_URL}${API_ENDPOINTS.blogs}/${id}/assets/${index}/name`
};
...
export default function EditBlog() {
  ...
  const fetchBlog = async () => {
    try {
      const response = await fetch(blogEndpoints.byId(blogId!));
      ...
      if (blog.mdFiles && blog.mdFiles.length > 0) {
        const mdResponse = await fetch(blogEndpoints.mdContent(blogId!));
  ...
  const handleSave = async () => {
    const password = window.prompt('Enter password to edit this blog');
    if (!password) {
      return;
    }
    ...
    try {
      ...
      const updateResponse = await fetch(blogEndpoints.byId(blogId!), {
        method: 'PUT',
  ...
  const uploadAssets = async (files: FileList) => {
    ...
    try {
      ...
      const response = await fetch(blogEndpoints.assets(blogId!), {
        method: 'POST',
        body: formData
      });
  ...
  const uploadCover = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(blogEndpoints.cover(blogId!), {
        method: 'POST',
        body: formData
      });
  ...
  const deleteAsset = async (index: number) => {
    ...
    try {
      const response = await fetch(blogEndpoints.assetByIndex(blogId!, index), {
        method: 'DELETE'
      });
  ...
  const updateAssetName = async (index: number, newName: string) => {
    try {
      const response = await fetch(blogEndpoints.assetName(blogId!, index), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
